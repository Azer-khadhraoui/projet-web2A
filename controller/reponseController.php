...
class ReponseController
{
    ...
    {
        $sql = "INSERT INTO responsereclamation (contenue, id_quest, id_user, date) VALUES (:contenue, :id_quest, :id_user, :date)";
        try {
    ...
    {
        $sql = "SELECT * FROM responsereclamation WHERE id_quest = :id_quest ORDER BY date ASC";
        try {
            $stmt = $this->db->prepare($sql);
...
*/
   ...
   /* public function getResponsesByQuestionId($id_quest)
    ...
    {
        $sql = "SELECT COUNT(*) FROM responsereclamation WHERE id_reponse = :id AND id_user = :id_user";
        try {
            $stmt = $this->db->prepare($sql);
    ...
    public function deleteResponse($id_response, $id_user)
    {
        if ($this->isUserResponseOwner($id_response, $id_user)) {
            $sql = "DELETE FROM responsereclamation WHERE id_reponse= :id";
            try {
                $stmt = $this->db->prepare($sql);
                $stmt->bindParam(':id', $id_response);
                $stmt->execute();
    ...
    {
        try {
            $query = $this->db->prepare("SELECT COUNT(*) AS total FROM responsereclamation");
            $query->execute();
    ...
    {
        if ($this->isUserResponseOwner($id_response, $id_user)) {
            $sql = "UPDATE responsereclamation SET contenue = :contenue WHERE id_reponse = :id";
            try {
                $stmt = $this->db->prepare($sql);
                $stmt->bindParam(':contenue', $contenue);
                $stmt->bindParam(':id', $id_response);
    ...
    public function countUserResponsesToQuestion($questionId, $userId)
    {
        try {
            ...
            $query = "SELECT COUNT(*) FROM responsereclamation WHERE id_quest = :id_quest AND id_user = :id_user";
    ...
    {
        $deleteQuery = "DELETE FROM responsereclamation WHERE id_reponse = ?";
    ...
    public function deleteResponsesWithBadWords()
    {
        ...
        $query = "SELECT id_reponse, contenue FROM responsereclamation WHERE " . implode(' OR ', array_map(fn($word) => "contenue LIKE ?", $badWords));
